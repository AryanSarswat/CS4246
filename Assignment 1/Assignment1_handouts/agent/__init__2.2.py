...
class GeneratePDDL_Stationary :
    ...
    def generateInitString(self) :
        ...
        agentString += " (atTime " + self.grid_cell_list[self.state.agent.position.x * self.num_lanes + self.state.agent.position.y] + " agent1 t0)"
        
        # Add time predicates
        timeString = ""
        prev = 0
        for time in range(1, self.width):
            timeString+= f" (isNextTime t{prev} t{time})"
            prev = time
        ...
        for car in self.state.cars :
            ...
            carSpeed = car.speed_range[0]
            
            for time in range(self.width):
                
                carString += " (atTime {} car{} t{})".format(self.grid_cell_list[carPosX * self.num_lanes + carPosY], car.id, time)
                
                carString += " (blocked {} t{})".format(self.grid_cell_list[carPosX * self.num_lanes + carPosY], time)
                
                if time < self.width - 1:
                    for speed in range(1, abs(carSpeed)):
                        if carPosX - speed >= 0:
                            carString += " (blocked {} t{})".format(self.grid_cell_list[(carPosX + carSpeed + speed) * self.num_lanes + carPosY], time + 1)
                
                carPosX += carSpeed
                if carPosX < 0 :
                    carPosX = self.width + carPosX
                    
                     
        
        # Initial State of grid cells with their up_next, down_next, forward_next_1, forward_next_2, forward_next_3 predicates
        gridString = ''
        for w in range(self.width) :
            for lane in range(self.num_lanes) :
                if w > 0 :
                    ...
                    gridString += " (forward_next_1 {} {})".format(self.grid_cell_list[w * self.num_lanes + lane], self.grid_cell_list[(w - 1) * self.num_lanes + lane])
                    
                    # To prevent the agent from going out of the grid
                    if w > 2:
                        gridString += " (forward_next_2 {} {})".format(self.grid_cell_list[w * self.num_lanes + lane], self.grid_cell_list[(w - 2) * self.num_lanes + lane])
                    
                    # To prevent the agent from going out of the grid
                    if w > 3:
                        gridString += " (forward_next_3 {} {})".format(self.grid_cell_list[w * self.num_lanes + lane], self.grid_cell_list[(w - 3) * self.num_lanes + lane])
        
        return agentString + " " + carString + gridString + timeString
...
def generateDomainPDDLFile(gen):
    ...
    gen.addPredicate(name="atTime", parameters=(("pt1" , "gridcell"), ("car", "car"), ("time", "time")))
    ...
    gen.addPredicate(name="forward_next_1", parameters=(("pt1" , "gridcell"), ("pt2", "gridcell")))
    gen.addPredicate(name="forward_next_2", parameters=(("pt1" , "gridcell"), ("pt2", "gridcell")))
    gen.addPredicate(name="forward_next_3", parameters=(("pt1" , "gridcell"), ("pt2", "gridcell")))
    gen.addPredicate(name="isNextTime", parameters=(("time1" , "time"), ("time2", "time")))
    gen.addPredicate(name="blocked", parameters=[("pt1" , "gridcell"), ("time", "time")] , isLastPredicate=True)
    
 
    gen.addAction(name="UP", 
                  parameters=(("src" , "gridcell"), ("dest", "gridcell"), ("agent", "agent"), ("srcTime", "time"), ("dstTime", "time")), 
                  precondition_string="(and (at ?src ?agent) (atTime ?src ?agent ?srcTime) (not (blocked ?dest ?dstTime)) (up_next ?src ?dest) (isNextTime ?srcTime ?dstTime))", 
                  effect_string="(and (at ?dest ?agent) (atTime ?dest ?agent ?dstTime) (not (at ?src ?agent)) (not (atTime ?src ?agent ?srcTime)))"
    )
    
    gen.addAction(name="DOWN", 
                  parameters=(("src" , "gridcell"), ("dest", "gridcell"), ("agent", "agent"), ("srcTime", "time"), ("dstTime", "time")), 
                  precondition_string="(and (at ?src ?agent) (atTime ?src ?agent ?srcTime) (not (blocked ?dest ?dstTime)) (down_next ?src ?dest) (isNextTime ?srcTime ?dstTime))", 
                  effect_string="(and (at ?dest ?agent) (atTime ?dest ?agent ?dstTime) (not (at ?src ?agent)) (not (atTime ?src ?agent ?srcTime)))"
    )
    
    gen.addAction(name="FORWARD_1", 
                  parameters=(("src" , "gridcell"), ("dest", "gridcell"), ("agent", "agent"), ("srcTime", "time"), ("dstTime", "time")), 
                  precondition_string="(and (at ?src ?agent) (atTime ?src ?agent ?srcTime) (not (blocked ?dest ?dstTime)) (forward_next_1 ?src ?dest) (isNextTime ?srcTime ?dstTime))", 
                  effect_string="(and (at ?dest ?agent) (atTime ?dest ?agent ?dstTime) (not (at ?src ?agent)) (not (atTime ?src ?agent ?srcTime)))"
    )
    
    gen.addAction(name="FORWARD_2", 
                  parameters=(("src" , "gridcell"), ("dest", "gridcell"), ("agent", "agent"), ("srcTime", "time"), ("dstTime", "time")), 
                  precondition_string="(and (at ?src ?agent) (atTime ?src ?agent ?srcTime) (not (blocked ?dest ?dstTime)) (forward_next_2 ?src ?dest) (isNextTime ?srcTime ?dstTime))", 
                  effect_string="(and (at ?dest ?agent) (atTime ?dest ?agent ?dstTime) (not (at ?src ?agent)) (not (atTime ?src ?agent ?srcTime)))"
    )
        
    gen.addAction(name="FORWARD_3", 
                  parameters=(("src" , "gridcell"), ("dest", "gridcell"), ("agent", "agent"), ("srcTime", "time"), ("dstTime", "time")), 
                  precondition_string="(and (at ?src ?agent) (atTime ?src ?agent ?srcTime) (not (blocked ?dest ?dstTime)) (forward_next_3 ?src ?dest) (isNextTime ?srcTime ?dstTime))", 
                  effect_string="(and (at ?dest ?agent) (atTime ?dest ?agent ?dstTime) (not (at ?src ?agent)) (not (atTime ?src ?agent ?srcTime)))"
    )
...
def generateProblemPDDLFile(gen):
    ...
    gen.addObjects("time", ["t{}".format(time) for time in range(gen.width)])
...
def test():
    ...
    test_configs['crossing'] = [{'lanes' : [LaneSpec(8, [-1, -1])] *2 + [LaneSpec(6, [-5, -5])] *2 +
                                           [LaneSpec(6, [-4, -4])] *2 + [LaneSpec(6, [-2, -2])] *2, 'width' :30, 'seed' : 101}]
